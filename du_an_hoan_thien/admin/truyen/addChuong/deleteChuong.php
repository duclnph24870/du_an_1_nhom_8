<?php 
    ...
    foreach ($idChuongs as $i) {
        ...
        $daDoc = select_all("SELECT * FROM dangdoc WHERE idChuong=$i");
        if (count($daDoc) == 1) {
            if ($truyen['soChuong'] == 1) { // nếu số chương bằng 1 thì xóa luôn đã đọc đi
                pdo_execute("DELETE FROM dangdoc WHERE idTruyen=$idTruyen");
            }elseif ($truyen['soChuong'] > 1) {
                ...
                pdo_execute("UPDATE `dangdoc` SET `idChuong`='".($numberChuongSau['idChuong'])."',`soChuong`=".$numberChuongSau['soChuong']." WHERE idTruyen=$idTruyen");
            }
        }
